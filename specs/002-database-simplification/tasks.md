...
- [X] T020 [P] [US2] Verify `api/turso-token.ts` (T007) responds well within a budget that
      ...
      Measured directly via `vercel curl` (built-in protection-bypass, no project-settings
      change needed) against the deployed Preview function: 5 requests to
      `POST /api/turso-token` with `x-guest-request: true`, warm round-trips 0.26–0.6s
      (cold start 1.2s), from this machine (Spain) to the function's `iad1` (US East)
      region. Noted as a real finding for T023: the function has no explicit `regions` in
      `vercel.json`, so it defaults to `iad1` even though both Turso (`aws-eu-west-1`) and
      Cognito/S3 (`eu-west-3`) are in Europe — a transatlantic hop on every token mint that
      the old S3 path never had. Did not change it since the end-to-end browser
...
- [X] T021 [US2] Manually measure time-to-data-visible on the summary page against the
      ...
      script rather than a DevTools walkthrough — see T022 for the harness). **Caveat**:
      the owner declined to pass real Cognito credentials to an automated script, so this
      specific real-user login flow was not separately timed end-to-end. The guest-path
      measurement in T022 is used as a representative proxy: after the token endpoint
      responds, the real-user and guest paths run identical client-side code (same
      libsql client, same summary-page queries); the only server-side difference is the
      Cognito JWT verification step, which is a local JWKS-cached check (cheap, not a
...
- [X] T022 [US2] Manually measure the same for the guest path, confirming no new failure
      ...
      Automated with a Puppeteer script (headless Chrome, `puppeteer-core` against the
      already-cached Chrome binary) driving the real guest-login flow end-to-end: load
      the login page → click "Guest" → click through to the Summary page → poll the DOM
      until the "Net" KPI card's value changes from its `0€` placeholder to a real number.
      Each run used a fresh isolated browser context (`createBrowserContext()`) so no
      localStorage auth state leaked between runs. Preview access used `vercel project
      protection enable ... --protection-bypass` (Vercel's official automation-bypass
      mechanism) for the duration of the test only; both generated bypass secrets were
      disabled again immediately after (confirmed via a follow-up 302 on the token
      endpoint — SSO protection is back on).

      5 isolated runs each, timed from clicking the Summary nav link to the KPI value
      updating:
      - Production (S3): `[495, 150, 211, 172, 185]` ms → median **185ms**
      - Preview (Turso): `[157, 181, 144, 139, 157]` ms → median **157ms**
...
- [X] T023 [US2] If either measurement regresses vs. baseline, address the specific
      ...
      No regression found (T022's numbers: Turso median 157ms vs. S3 median 185ms), so no
      fix was required. The `iad1` function-region mismatch noted in T020 is left as a
      known, currently-harmless inefficiency rather than "fixed" — recorded here in case a
      future measurement (e.g. under real-world network conditions rather than this
      machine's) does show it mattering.
