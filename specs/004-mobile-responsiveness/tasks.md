...
- [ ] T010 [US3] Replace `useWindowSize()`'s `window`-resize listener with a `ResizeObserver` observing the chart's own container element, in `src/common/utils.ts`, keeping the existing hook signature/return shape so call sites don't need structural changes.
- [ ] T011 [US3] Add tiered chart margins (a smaller margin set below the `md:` width threshold) to `src/routes/summary/-plots/IncomeExpensesPlot.tsx`.
- [ ] T012 [P] [US3] Apply the same tiered-margin treatment to the remaining D3 chart files under `src/routes/summary/-plots/` (`DetailedExpensesBarPlot.tsx`, `DetailedIncomeBarPlot.tsx`, `MonthDetailedExpensesPiePlot .tsx`, `AssetAccountsPlot.tsx`, and any other sibling chart files in that directory using fixed pixel margins).
- [ ] T013 [P] [US3] Apply the same tiered-margin treatment to the D3 chart files under `src/routes/travels/-components/` (`TravelExpensesMonthlyPlot.tsx`, `TravelExpensesDetailedPlot.tsx`, `TravelExpensesPlot.tsx`, `TravelExpensesPiePlot .tsx`).
- [ ] T014 [US3] Update `src/routes/summary/-plots/Tooltip.tsx` to use `useIsTouchDevice()` instead of `isMobile()` for its hide-delay timing.
- [ ] T015 [US3] Review `src/components/charts/BarPlot.tsx`'s fixed `h-80` container height at narrow viewports (visually, during T016) and reduce it responsively if it's disproportionate on a 320-375px screen.
