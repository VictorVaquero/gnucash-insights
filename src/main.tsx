...
const GlobalCOntextProvider = () => {
  ...
  const { data: queryFolders, isError: isFolderError } = useQuery(
    awsFolderOptions({ user: auth.user, credentials: auth.getCredentials() })
  );
  const {
    data: queryDb,
    isError: isDBError,
  ...
  useEffect(() => {
    if (!!queryFolders && !folders) {
      ...
      const parseDate = (dt: string): string =>
        ...
        "";
      const fileOptions = dbList.map((item) => ({
        key: item,
  ...
  console.info(
    `Current files ${fileName} book ${bookId} db ${!!db} user ${auth.user}`
  );
