...
export const fetchDBOptions = ({ fileName, user, credentials }: { fileName: string | undefined, user: string | undefined, credentials: CognitoIdentityCredentialProvider | undefined }) => {
    const folderPath = user === 'guest' ? config.guestFolderPath : config.folderPath;
    const options = awsReadDbOptions({ object: `${folderPath}${fileName}/cash.db`, user, credentials, enabled: !!fileName })
    return options
}