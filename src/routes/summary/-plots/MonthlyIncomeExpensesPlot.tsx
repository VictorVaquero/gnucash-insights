...
import { incomeExpensesYearMonthOptions, profitLossYearMonthOptions, taxesYearMonthOptions } from "@/db/queries/summary";
...
const colorCodes: Record<colorType, string> = {
    'g': fullTWConfig.theme.colors.emerald[500],
    'r': fullTWConfig.theme.colors.red[500]
}
    ...
    return arrs[0].map(itemA => {
        return arrs.reduce((acum, arr) => {
            const matchingItem = arr.find(item =>
                commonKeys.every((key) => { return acum[key as never] === item[key as keyof typeof item]; })
            );
            if (!matchingItem) {
...
const DrawMonthlyIncomeExpensesPlot = (props: { data: Data[], profit: Data[], domain: { startDate: DateTime, endDate: DateTime } }) => {
    ...
    const mixin = [
        sortedData.filter((d) => d.type === 'r').map((d) => ({ ...d, expenses: d.value })),
        sortedProfit.map((d) => ({ ...d, net: d.value })),
        sortedData.filter((d) => d.type === 'g').map((d) => ({ ...d, income: d.value }))
    ]
    ...
    const choosePoint = chooseTooltipPointLine(joined as unknown as Mixin[], xf, yf, xScale, yScale);
    const updateTooltip = (ref: MutableRefObject<HTMLDivElement | null>, d: Mixin) => {
        if (ref.current !== null) {
    ...
    return <div className='relative w-full h-full'>
        <svg className='w-full h-full' ref={svgRef}>
            ...
            <g className='circles'>
                {sortedData.map((d) =>
                    <circle fill={getColor(d.type)}
                        key={d.name + d.yearmonth}
                        strokeWidth='1.5' shapeRendering='geometricPrecision' stroke='white' r='5'
                        cx={xScale(new Date(d.yearmonth))} cy={yScale(d.value)}
                    />
                )}
            </g>
        </svg>
        <Tooltip svgRef={svgRef} choosePoint={choosePoint} updateTooltip={updateTooltip}>
            <div className='flex flex-col items-center px-6 py-2'>
                ...
                <div className='text-shark-500'>Income: <span id='income' className="text-emerald-500">Income</span></div>
                <div className='text-shark-500'>Expenses: <span id='expenses' className="text-red-500">Expenses</span></div>
                <div className='text-shark-500'>Net: <span id='net'>net</span></div>
            </div>
        </Tooltip>
    </div>
        ;
...
export const MonthlyIncomeExpensesPlot = () => {
    ...
    const { data: dataFull, isSuccess } = useQuery(incomeExpensesYearMonthOptions({ db, user, bookId }));
    const { data: taxes, isSuccess: isSuccessTaxes } = useQuery(taxesYearMonthOptions({ db, user, bookId }));
    const { data: profit, isSuccess: isSuccessProfit } = useQuery(profitLossYearMonthOptions({ db, user, bookId }));
    const data = useMemo(() => { if (isSuccess && isSuccessTaxes) return dataFull.map((d) => ({ ...d, value: d.value - (taxes.find((t) => t.yearmonth === d.yearmonth)?.value || 0) })) }, [dataFull, taxes, isSuccess, isSuccessTaxes]);

    if (!data || !isSuccessProfit || !startDate || !endDate) return <div className='w-full h-full flex flex-row items-center justify-center'><BarLoader color='#36d7b7' /></div>

    return <DrawMonthlyIncomeExpensesPlot data={data as Data[]} profit={profit as Data[]} domain={{ startDate, endDate }} />
}
