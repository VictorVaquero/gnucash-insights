...
const DrawMonthlyDetailedExpenses = (props: {
    ...
    const yDomain = [0, d3.max(series.map((s)=>s.map((d)=>d[1])).flat())!]; // TODO: Fix
    ...
    const onClick = (d: NetCostsYearMonth) => props.setDate(DateTime.fromISO(d.date));
    ...
    return <div className="h-full flex flex-col">
        ...
        <div className="h-full">
            <DrawMonthlyDetailedExpenses
                ...
                domain={{ startDate: isYearly ? domain.startDate.minus({ 'month': 8 }) : domain.startDate, endDate: domain.endDate }}
...
function nestCollapse(data: NetCostsYearMonth[], limit: number): NetCostsYearMonth[] {
    ...
    const collapsed_data = groupCollapse<NetCostsYearMonth>(data, limit, (d) => d.value, (d) => d.account, (d) => d.date)
