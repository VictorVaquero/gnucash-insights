...
const DrawMonthlyAccountsPlot = (props: { data: Data[], domain: { startDate: DateTime, endDate: DateTime } }) => {
    ...
    const choosePoint = chooseTooltipPointLine<Data>(sortedData, xf, yf, xScale, yScale);
...
export const MonthlyAccountsPlot = () => {
    ...
    const { data, isSuccess } = useQuery(assetsDebtsYearMonthOptions({ db, user, bookId }));

    if (!isSuccess || !startDate || !endDate) return <div className='w-full h-full flex flex-row items-center justify-center'><BarLoader color='#36d7b7' /></div>

    return <DrawMonthlyAccountsPlot data={data} domain={{ startDate, endDate }} />
}
