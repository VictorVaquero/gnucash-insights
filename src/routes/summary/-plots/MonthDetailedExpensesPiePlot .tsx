...
export const DrawMonthDetailedExpensesPiePlot = (props: { data: Data[], accounts: Account[], date: DateTime, hideAccounts: string[], setHideAccounts: CallableFunction }) => {
    ...
    const color_f = (d: Data) => name_f(d) !== defaultAccount ? getRandomColor(name_f(d)) : getDefaultColor();
...
export const MonthDetailedExpensesPiePlot = (props: { date: DateTime; hideAccounts: string[]; setHideAccounts: CallableFunction; }) => {
    const { db } = useDB();
    ...
    if (!isSuccessAccounts || !isSuccess) return <div className='w-full h-full flex flex-row items-center justify-center'> <BarLoader color='#36d7b7' /> </div>;

    return <DrawMonthDetailedExpensesPiePlot data={data} accounts={accounts} date={props.date} hideAccounts={props.hideAccounts} setHideAccounts={props.setHideAccounts} />;
};