...
export const TravelExpensesMonthlyPlot = () => {
    ...
    const { data: dataYearly, isSuccess: isSuccessYearly } = useQuery(travelExpensesYearOptions({ db, user, bookId }));

    if (!isSuccess || !isSuccessYearly || !min || !max) return <div className='w-full h-full flex flex-row items-center justify-center'><BarLoader color='#36d7b7' /></div>
