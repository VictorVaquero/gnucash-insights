...
export const KpiBlock = (props: { data: FullTransaction[] }) => {
    ...
    const xf = (d: FullTransaction) => d.transactions.datePosted;
    const groupedData = d3.groups(props.data, d => xf(d).toFormat('yyyy-LL')).map(([, data]) => ({
        'date': xf(data[0]),
    ...
    const meanAllTime = props.data.reduce((v, d) => v + d.splits.value, 0) / (numMonths ?? 1);
    const meanLastMonth = sortedData.filter((d) => d.date === latestMonth).reduce((v, d) => v + d.value, 0);
    const meanLast3Months = sortedData.filter((d) => d.date < latestMonth && d.date >= latestMonth.minus({ month: 3 })).reduce((v, d) => v + d.value, 0) / 3;
    const meanLast6Months = sortedData.filter((d) => d.date < latestMonth && d.date >= latestMonth.minus({ month: 6 })).reduce((v, d) => v + d.value, 0) / 6;
    const meanLastYear = sortedData.filter((d) => d.date < latestMonth && d.date >= latestMonth.minus({ month: 12 })).reduce((v, d) => v + d.value, 0) / 12;
