...
import {XAxis} from "@/routes/summary/-plots/XAxis";
import {YAxis} from "@/routes/summary/-plots/YAxis";
...
export const TransactsPlot = (props: {data: FullTransaction[], isYearly: boolean}) => {
    ...
    const groupedData = d3.groups(props.data, d => d.transactions.datePosted.toFormat(format)).map(([date, data])=> ({
    ...
    const line = d3.line<GroupedTransaction>()
        .curve(d3.curveMonotoneX)
        .x((d) => xScale(xf(d)))
        .y((d) => yScale(yf(d)));

    const choosePoint = chooseTooltipPointLine<GroupedTransaction>(sortedData, xf, yf, xScale, yScale);
    const updateTooltip = (ref: MutableRefObject<HTMLDivElement|null>, d: GroupedTransaction) => {
        if(ref.current !== null) {
