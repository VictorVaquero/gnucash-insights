...
const xf = (d: FullTransaction | GroupedTransaction) => DateTime.fromISO(d.posted);
const yf = (d: FullTransaction | GroupedTransaction) => d.value;
const orderxf = (a: FullTransaction | GroupedTransaction, b: FullTransaction | GroupedTransaction) => xf(a) > xf(b) ? 1 : -1;
const orderyf = (a: FullTransaction | GroupedTransaction, b: FullTransaction | GroupedTransaction) => yf(a) > yf(b) ? 1 : -1;

export const TransactsPlot = (props: {data: FullTransaction[]}) => {
    ...
    const line = d3.line<FullTransaction | GroupedTransaction>()
        .curve(d3.curveLinear)
        .x((d) => xScale(xf(d)))
        .y((d) => yScale(yf(d)));

    const choosePoint = chooseTooltipPointLine<FullTransaction | GroupedTransaction>(sortedData, xf, yf, xScale, yScale);
    const updateTooltip = (ref: MutableRefObject<HTMLDivElement|null>, d: FullTransaction | GroupedTransaction) => {
        if(ref.current !== null) {
            const tooltip = d3.select(ref.current)
            tooltip.select('#title').text(d.posted)
