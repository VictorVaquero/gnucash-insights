...
const getTravelExpensesDetailedQuery = ({
  ...
  return db
    .select({
      name: sql<string>`${ft.slNotes}`.as("name"),
...
export const travelExpensesDetailedYearMonthOptions = ({
  ...
  return queryOptions({
    ...
    queryFn: !enabled
      ? skipToken
      : async () =>
          getTravelExpensesDetailedYearMonthQuery({
            db,
            user,
            bookId,
          }).execute(),
...
export const travelExpensesYearOptions = ({
  ...
  return queryOptions({
    ...
    queryFn: !enabled
      ? skipToken
      : async () => getTravelExpensesYearQuery({ db, user, bookId }).execute(),
    enabled: enabled,
  });
};
...
const getTravelExpenseKPIsQuery = ({
  ...
  return db
    .select({
      ...
      expense_all: sql<number>`sum(CASE WHEN substr(${
        transactionsTable.slNotes
      }, 0, ${dbconf.tripDesc.length + 1}) = ${dbconf.tripDesc} THEN ${
        splitsTable.value
      } ELSE 0 END) `,
    })
...
export const useGetTravelExpensesKPIs = ({
  ...
  return useQuery({
    ...
    queryFn: !enabled
      ? skipToken
      : async () => {
          const data = await getTravelExpenseKPIsQuery({
            db,
            bookId,
            user,
            latestMonth,
...
const getUniqueTravelsQuery = ({
  ...
  return db
    .select({
      number: countDistinct(ft.slNotes),
    })
    .from(ft)
    .innerJoin(accountsFiltered, eq(accountsFiltered.id, ft.accountId))
    .innerJoin(
