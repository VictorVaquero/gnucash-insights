...
export const DropDownForm = (
    props: {id: string, label: string, list: DropDownItem[], value: string, setValue: CallableFunction}) => {
    return <form className='p-4 pt-2.5 bg-shark-700 w-max rounded'>
        ...
        >
            {props.list.map((item) => <option key={item.key} value={item.value}>{item.value}</option>)}
        </select>
    </form>
}